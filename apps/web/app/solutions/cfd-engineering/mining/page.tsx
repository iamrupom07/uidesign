...
import { MINING_SERVICES, MiningService } from "@/lib/mining-data";
import {
  ArrowRight,
  CheckCircle2,
  Cpu,
...
const WHY_CHOOSE_SIMULATION = [
  {
    title: "Deep Mining & Mineral Processing Expertise",
    desc: "Specialized mining, slurry, and particle dynamics engineers with extensive experience across ore transport, transfer chutes, hydrocyclone clusters, and underground mine ventilation.",
    icon: Factory,
  },
  {
    title: "High-Fidelity DEM & Slurry Physics",
    desc: "Coupled CFD-DEM particle modeling of bulk solids, discrete particle-air interaction, slurry settling, critical transport velocity, and abrasive impingement erosion.",
    icon: Flame,
  },
  {
    title: "Zero Shutdown Virtual Optimization",
    desc: "Test transfer chute geometries, hydrocyclone vortex finders, dust extraction hoods, and mine air route modifications in a virtual environment prior to physical plant changes.",
    icon: Cpu,
  },
  {
    title: "Actionable Engineering Deliverables",
    desc: "Clear visual particle trajectory maps, erosion depth contours (mm/year), pressure drop calculations, and CAD-ready chute/pipeline modification drawings.",
    icon: FileText,
  },
  {
    title: "Erosion Mitigation & Liner Life Extension",
    desc: "Predict localized wall shear and particle impact angles to optimize wear-liner placement, reduce maintenance frequency, and prevent catastrophic pipeline pipe burst events.",
    icon: Zap,
  },
  {
    title: "Proven Debottlenecking Track Record",
    desc: "Consistently resolving transfer chute plugging, slurry pipe blockage, mine air recirculation, dust escape, and hydrocyclone coarse particle bypass in major mining operations worldwide.",
    icon: Award,
  },
];

export default function MiningCfdPage() {
  const [activeServiceId, setActiveServiceId] = useState<string>("01");

  const activeService =
    MINING_SERVICES.find((s) => s.id === activeServiceId) || MINING_SERVICES[0];
          ...
          <div className="border border-border p-8 sm:p-12 lg:p-16 bg-white relative overflow-hidden shadow-sm">
            ...
            <Reveal>
              <div className="font-mono text-[11px] font-bold text-primary tracking-widest uppercase mb-4 flex items-center gap-2">
                ...
                <span>/</span>
                <span className="text-foreground">MINING INDUSTRY</span>
              </div>
              ...
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-rose-50 border border-rose-200 text-primary font-mono text-[10px] font-bold uppercase tracking-wider mb-6">
                ...
                <span>MACPROTEC Specialized Mining Expertise Center</span>
              </div>
              ...
              <p className="body-md text-secondary max-w-4xl leading-relaxed font-sans mb-8">
                MACPROTEC provides comprehensive Computational Fluid Dynamics (CFD), DEM particle transport, slurry pipeline hydraulics,
                hydrocyclone classification, dust extraction & suppression, mine ventilation airflow modeling, erosion & wear prediction,
                and root cause failure diagnostics tailored specifically to mining operations and mineral processing plants.
              </p>
        ...
        <section id="service-cards" className="max-w-7xl mx-auto px-6 lg:px-8 space-y-8">
          <Reveal>
            <div className="font-mono text-[11px] font-bold text-primary tracking-widest uppercase mb-2">
              <span className="text-primary font-bold mr-1">┌</span> APPLIED MINING ENGINEERING SERVICES
            </div>
