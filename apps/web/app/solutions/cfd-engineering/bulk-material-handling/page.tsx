...
import { BULK_MATERIAL_HANDLING_SERVICES, BulkMaterialHandlingService } from "@/lib/bulk-material-handling-data";
import {
  ArrowRight,
  CheckCircle2,
  Cpu,
...
const WHY_CHOOSE_SIMULATION = [
  {
    title: "Deep Bulk Material Handling Domain Expertise",
    desc: "Specialized material handling, pneumatic transport, and particle dynamics engineers with extensive experience across transfer chutes, silos, hoppers, and port terminals.",
    icon: Factory,
  },
  {
    title: "High-Fidelity DEM & CFD-DEM Physics",
    desc: "Coupled CFD-DEM particle modeling of bulk solids trajectories, particle segregation, dense/dilute phase gas-solid transport, and impact energy distribution.",
    icon: Flame,
  },
  {
    title: "Zero Risk Virtual Design Validation",
    desc: "Test transfer chute geometry modifications, hopper cone angles, dust extraction hood placement, and pneumatic line velocities in a virtual simulation environment.",
    icon: Cpu,
  },
  {
    title: "Actionable Engineering Deliverables",
    desc: "Clear visual particle velocity maps, material build-up zone identification, erosion rate contours (mm/year), and CAD-ready chute/hopper modification drawings.",
    icon: FileText,
  },
  {
    title: "Erosion & Belt Damage Prevention",
    desc: "Predict particle impact angles and material centering to optimize wear-liner placement, prevent off-center conveyor loading, and extend belt lifespan.",
    icon: Zap,
  },
  {
    title: "Proven Debottlenecking Track Record",
    desc: "Consistently resolving transfer chute plugging, silo ratholing/arching, pneumatic pipe blockage, fugitive dust escape, and belt wear in major bulk handling facilities worldwide.",
    icon: Award,
  },
];

export default function BulkMaterialHandlingCfdPage() {
  const [activeServiceId, setActiveServiceId] = useState<string>("01");

  const activeService =
    BULK_MATERIAL_HANDLING_SERVICES.find((s) => s.id === activeServiceId) || BULK_MATERIAL_HANDLING_SERVICES[0];
        ...
        <section className="max-w-7xl mx-auto px-6 lg:px-8 pt-6">
          <div className="border border-border p-8 sm:p-12 lg:p-16 bg-white relative overflow-hidden shadow-sm">
            ...
            <div className="absolute top-4 right-4 font-mono text-[9px] text-slate-400">
              SECTOR DOSSIER / BULK-MATERIAL-HANDLING-CFD
            </div>
            ...
            <Reveal>
              <div className="font-mono text-[11px] font-bold text-primary tracking-widest uppercase mb-4 flex items-center gap-2">
                ...
                <span>/</span>
                <span className="text-foreground">BULK MATERIAL HANDLING</span>
              </div>
              ...
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-rose-50 border border-rose-200 text-primary font-mono text-[10px] font-bold uppercase tracking-wider mb-6">
                ...
                <span>MACPROTEC Specialized Bulk Material Expertise Center</span>
              </div>
              ...
              <p className="body-md text-secondary max-w-4xl leading-relaxed font-sans mb-8">
                MACPROTEC provides comprehensive Computational Fluid Dynamics (CFD), DEM particle transport, pneumatic conveying,
                transfer chute dynamics, hopper/silo discharge optimization, dust extraction & suppression, erosion modeling,
                and root cause failure diagnostics tailored specifically to bulk material handling systems.
              </p>
        ...
        <section id="service-cards" className="max-w-7xl mx-auto px-6 lg:px-8 space-y-8">
          <Reveal>
            <div className="font-mono text-[11px] font-bold text-primary tracking-widest uppercase mb-2">
              <span className="text-primary font-bold mr-1">┌</span> APPLIED BULK MATERIAL HANDLING SERVICES
            </div>
          ...
          <RevealGroup className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6" stagger={0.04}>
            {BULK_MATERIAL_HANDLING_SERVICES.map((svc) => (
        ...
        <section id="detailed-dossier" className="max-w-7xl mx-auto px-6 lg:px-8 space-y-8 pt-10">
          ...
          <Reveal>
            <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none border-b border-border">
              {BULK_MATERIAL_HANDLING_SERVICES.map((s) => (
                <button
                  key={s.id}
                ...
                >
                  ...
                  <span>{s.title.replace(" CFD Analysis", "").replace(" Analysis", "").replace(" & Wear Analysis", "")}</span>
                </button>
              ))}
            </div>
          </Reveal>
                ...
                <div className="lg:col-span-5">
                  <div className="border border-border p-2 bg-slate-50 relative group">
                    ...
                    <div className="p-3 bg-white border-t border-border flex items-center justify-between font-mono text-[10px] text-slate-500">
                      ...
                      <span className="text-primary font-bold">HIGH FIDELITY CFD/DEM</span>
                    </div>
                  </div>
                </div>
              </div>
