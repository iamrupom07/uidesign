...
import { CHEMICAL_SERVICES, ChemicalService } from "@/lib/chemical-data";
import {
  ArrowRight,
  CheckCircle2,
  Cpu,
...
const WHY_CHOOSE_SIMULATION = [
  {
    title: "Deep Chemical Industry Domain Expertise",
    desc: "Specialized chemical process and fluid dynamics engineers with extensive experience across stirred tank reactors, multiphase contactors, heat exchangers, and toxic gas dispersion.",
    icon: Factory,
  },
  {
    title: "High-Fidelity Reaction & Multiphase Physics",
    desc: "Advanced CFD modeling of species transport, reaction kinetics, gas-liquid mass transfer, particle suspension, and residence time distribution (RTD).",
    icon: Flame,
  },
  {
    title: "Zero Shutdown Virtual Optimization",
    desc: "Test agitator impeller designs, feed injection locations, jacket cooling configurations, and ventilation layouts in a virtual CFD environment prior to physical plant changes.",
    icon: Cpu,
  },
  {
    title: "Actionable Engineering Deliverables",
    desc: "Clear visual concentration maps, velocity streamlines, temperature contours, ASME code pipe stress reports, and CAD-ready equipment modification drawings.",
    icon: FileText,
  },
  {
    title: "Process Safety & Hazard Control",
    desc: "Targeted toxic and flammable gas dispersion modeling, hazardous accumulation identification, and gas detector placement optimization for safer plant operation.",
    icon: Zap,
  },
  {
    title: "Proven Debottlenecking Track Record",
    desc: "Consistently resolving poor reactor mixing, liquid carryover, heat exchanger fouling, local hot spots, and pipe stress overload in chemical processing facilities.",
    icon: Award,
  },
];

export default function ChemicalCfdPage() {
  const [activeServiceId, setActiveServiceId] = useState<string>("01");

  const activeService =
    CHEMICAL_SERVICES.find((s) => s.id === activeServiceId) || CHEMICAL_SERVICES[0];
          ...
          <div className="border border-border p-8 sm:p-12 lg:p-16 bg-white relative overflow-hidden shadow-sm">
            ...
            <Reveal>
              <div className="font-mono text-[11px] font-bold text-primary tracking-widest uppercase mb-4 flex items-center gap-2">
                ...
                <span>/</span>
                <span className="text-foreground">CHEMICAL INDUSTRY</span>
              </div>
              ...
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-rose-50 border border-rose-200 text-primary font-mono text-[10px] font-bold uppercase tracking-wider mb-6">
                ...
                <span>MACPROTEC Specialized Chemical Expertise Center</span>
              </div>
              ...
              <p className="body-md text-secondary max-w-4xl leading-relaxed font-sans mb-8">
                MACPROTEC provides comprehensive Computational Fluid Dynamics (CFD), reactor mixing, multiphase reactive flow,
                thermal management, heat exchanger flow distribution, industrial gas dispersion ventilation, pipe stress analysis,
        ...
        <section id="service-cards" className="max-w-7xl mx-auto px-6 lg:px-8 space-y-8">
          <Reveal>
            <div className="font-mono text-[11px] font-bold text-primary tracking-widest uppercase mb-2">
              <span className="text-primary font-bold mr-1">┌</span> APPLIED CHEMICAL ENGINEERING SERVICES
            </div>
