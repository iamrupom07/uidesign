...
import { CEMENT_SERVICES, CementService } from "@/lib/cement-data";
import {
  ArrowRight,
  CheckCircle2,
  Cpu,
...
export default function CementCfdPage() {
  const [activeServiceId, setActiveServiceId] = useState<string>("01");

  const activeService =
    CEMENT_SERVICES.find((s) => s.id === activeServiceId) || CEMENT_SERVICES[0];
  ...
  return (
    <>
      ...
      <main className="bg-background min-h-screen py-20 lg:py-24 space-y-20">
          ...
          <div className="border border-border p-8 sm:p-12 lg:p-16 bg-white relative overflow-hidden shadow-sm">
            ...
            <Reveal>
              <div className="font-mono text-[11px] font-bold text-primary tracking-widest uppercase mb-4 flex items-center gap-2">
                ...
                <span>/</span>
                <span className="text-foreground">CEMENT INDUSTRY</span>
              </div>
              ...
              <p className="body-md text-secondary max-w-4xl leading-relaxed font-sans mb-8">
                The cement industry is MACPROTEC's primary core area of expertise.
                We provide comprehensive Computational Fluid Dynamics (CFD), heat transfer,
                combustion, multiphase flow, dust control, and structural pipe stress analysis
        ...
        {/* Engineering Services Cards Grid (Page 1 of PDF Instruction) */}
        <section id="service-cards" className="max-w-7xl mx-auto px-6 lg:px-8 space-y-8">
          <Reveal>
            <div className="font-mono text-[11px] font-bold text-primary tracking-widest uppercase mb-2">
              <span className="text-primary font-bold mr-1">┌</span> APPLIED CEMENT ENGINEERING SERVICES
            </div>
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none border-b border-border">
              {CEMENT_SERVICES.map((s) => (
                ...
                >
                  ...
                  <span>{s.title.replace(" CFD Analysis", "").replace(" Analysis & Support Design", "")}</span>
                </button>
              ))}
            </div>
          </Reveal>
                  ...
                  <div className="border border-border p-2 bg-slate-50 relative group">
                    <div className="relative h-64 sm:h-72 w-full bg-slate-900 overflow-hidden border border-border">
                      ...
                      <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between font-mono text-[9px] text-white bg-slate-950/80 p-2 border border-slate-800 backdrop-blur-sm">
                        <span>MODEL: MACPROTEC-SIM-{activeService.id}</span>
        ...
        <section className="max-w-7xl mx-auto px-6 lg:px-8 space-y-8">
          <Reveal>
            <div className="font-mono text-[11px] font-bold text-primary tracking-widest uppercase mb-2">
              <span className="text-primary font-bold mr-1">┌</span> CEMENT PLANT MODULES
            </div>
            <h2 className="font-display font-extrabold text-3xl sm:text-4xl text-foreground uppercase tracking-tight">
              Cement Plant Equipment <span className="text-primary">Applications</span>
            </h2>
            <p className="font-mono text-sm font-semibold text-secondary max-w-3xl mt-2">
          ...
          <RevealGroup className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6" stagger={0.04}>
            {[
              {
                title: "Rotary Kiln & Main Burner CFD",
                desc: "Flame shape optimization, fuel-air mixing, refractory heat loss evaluation, and thermal profile modeling.",
                tag: "PYROPROCESSING",
              },
              {
                title: "Calciner & Preheater Cyclones",
                desc: "Raw meal separation efficiency, calcination residence time, pressure drop reduction, and false air detection.",
                tag: "PREHEATER TOWER",
              },
              {
                title: "Clinker Cooler CFD",
                desc: "Clinker bed airflow distribution, heat recovery optimization, secondary air temp maximization, and fan energy reduction.",
                tag: "COOLER SYSTEM",
              },
              {
                title: "Vertical Roller & Ball Mill CFD",
                desc: "Internal mill gas flow, classifier separation efficiency, pressure drop tuning, and material transport optimization.",
                tag: "GRINDING CIRCUITS",
              },
              {
                title: "Dynamic Separators & Classifiers",
                desc: "Multiphase particle trajectory simulation, cut-size tuning, and circulating load reduction.",
                tag: "SEPARATION",
              },
              {
                title: "Bag Filters & ESP Dust Collectors",
                desc: "Flow uniformity across filter bags, dust loading distribution, and pressure drop minimization.",
                tag: "DUST CONTROL",
              },
              {
                title: "Pneumatic Conveying Lines",
                desc: "Air-solid velocity optimization, pipeline wear prediction, deposition prevention, and blockage risk mitigation.",
                tag: "BULK CONVEYING",
              },
              {
                title: "High-Temperature Ducting & Piping",
                desc: "Pipe stress FEA, thermal expansion joints, support design, and nozzle load verification.",
                tag: "PIPING INTEGRITY",
              },
              {
                title: "Alternative Fuel Co-Processing",
                desc: "RDF, SRF, and biomass burnout modeling, chlorine bypass optimization, and combustion stability.",
                tag: "DECARBONIZATION",
              },
            ].map((app, idx) => (
              <RevealItem key={idx}>
