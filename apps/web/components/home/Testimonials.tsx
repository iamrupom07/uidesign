    ...
    <section className="py-12 sm:py-xl bg-background border-t border-border blueprint-mesh">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <Reveal className="text-center max-w-2xl mx-auto mb-12 sm:mb-14">
          <div className="font-mono text-xs font-bold text-primary tracking-widest uppercase mb-3 justify-center flex">
        ...
        <div className="grid md:grid-cols-3 gap-6 lg:gap-lg">
          {list.map((item, index) => (
            <Reveal key={item.author} delay={index * 0.1}>
              <div className="card p-5 sm:p-8 relative h-full flex flex-col justify-between overflow-hidden">
                <p className="text-sm text-foreground font-sans italic leading-relaxed relative z-10">
                  &ldquo;{item.quote}&rdquo;
                </p>
                <div className="mt-8 flex items-center gap-3 relative z-10">
                  {/* Square placeholder avatar block */}
                  <div className="w-9 h-9 bg-rose-50 border border-rose-100 flex items-center justify-center font-mono text-xs font-bold text-primary rounded-none shrink-0">
                    {item.author[0]}
                  </div>
                  <div>
                    <p className="font-sans font-bold text-xs text-foreground uppercase">
                      {item.author}
                    </p>
                    <p className="font-sans text-[10px] text-secondary uppercase tracking-wider">
                      {item.role}
                    </p>
                  </div>
                </div>
