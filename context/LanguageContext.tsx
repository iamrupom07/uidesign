...
export function LanguageProvider({ children }: { children: React.ReactNode }) {
  ...
  const currentLangObj =
    LANGUAGES.find(
      (l) => l.code.toLowerCase() === language.toLowerCase() || l.code.split("-")[0].toLowerCase() === language.toLowerCase()
    ) || defaultLangObj;
  ...
  useEffect(() => {
    ...
    if (saved && LANGUAGES.some((l) => l.code === saved)) {
      ...
      const found = LANGUAGES.find((l) => l.code === cookieLang || l.code.split("-")[0] === cookieLang);
